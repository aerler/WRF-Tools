...
# script to rerun setup script for all experiments (subfolders with xconfig.sh file)
...
if [ -z $( getopt -T ) ]; then
  TMP=$( getopt -o vqrth --long wrftools:,errorlog:,verbose,quiet,running-only,test,help -n "$0" -- "$@" ) # pre-process arguments
...
while true; do
  case "$1" in
    ...
    -r | --running-only )   RUNONLY=1;   shift;;
    -t | --test         )   TEST=1;        shift;;
    -h | --help         )   echo -e " \
                            \n\
    ...
    -r | --running-only  only run setup for currently running experiments (sources 'running_experiments.sh') \n\
...
if [ ${RUNONLY} -eq 1 ]; then 
  source ${HOME}/running_experiments.sh
  DIRLIST=${ALL_JOBS}
else DIRLIST=$(ls -d */); fi
# loop over subfolders
for EE in ${DIRLIST};do 
