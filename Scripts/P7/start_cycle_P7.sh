...
export GEOGRID=${GEOGRID:-'ssh gpc04 "cd ${INIDIR}; source setup_GPC.sh; mpirun -n 4 ./geogrid.exe" > /dev/null'} # hide stdout; run on GPC via ssh
# 'mpiexec -n 8 ./geogrid.exe > /dev/null' # hide stdout # run locally
