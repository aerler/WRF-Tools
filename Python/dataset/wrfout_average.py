...
def processFileList(pid, filelist, filetype, ndom):
  ...
  if True: 
    if os.path.exists(meanfile):
      os.remove(meanfile)
#         mean = nc.Dataset(meanfile, mode='rw', format='NETCDF4')
...
#       else:        
    ...
    mean.createDimension(u'time', size=None) # make time dimension unlimited
    add_coord(mean, u'time', data=None, dtype='i4', atts=dict(units='month since '+begindate)) # unlimited time dimension
  ...
  if 'XTIME' in wrfout.variables: 
    ...
    assert wrfout.variables['XTIME'].description == "minutes since simulation start"
  elif 'Times' in wrfout.variables: 
...
#         print '%s-01_00:00:00'%(currentdate,),str().join(wrfout.variables['Times'][wrfstartidx,:])
    ...
    if lxtime: xtime = -1 * wrfout.variables['XTIME'][wrfstartidx] # seconds
      # N.B.: the first value is saved as negative, so that adding the last value yields a positive interval
    else: xtime = str().join(wrfout.variables['Times'][wrfstartidx,:]) # datestring of format '%Y-%m-%d_%H:%M:%S'
    ...
    while lincomplete:
      ...
      if lincomplete == False: 
        if lxtime:
          xtime += wrfout.variables['XTIME'][wrfendidx] # get final time interval (in minutes)
          xtime *=  60. # convert minutes to seconds   
        else: 
          dt1 = datetime.strptime(xtime, '%Y-%m-%d_%H:%M:%S')
          dt2 = datetime.strptime(str().join(wrfout.variables['Times'][wrfendidx,:]), '%Y-%m-%d_%H:%M:%S')
          ...
          yyyy, mm, dd = str().join(wrfout.variables['Times'][wrfendidx-1,0:10]).split('-')
